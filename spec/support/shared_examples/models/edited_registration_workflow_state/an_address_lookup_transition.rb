...
RSpec.shared_examples "an address lookup transition" do |next_state_without_skipping_to_manual:, address_type:, factory: :transient_registration|
  describe "#workflow_state" do
    ...
    context "when subject.skip_to_manual_address? is false" do
      next_state = next_state_without_skipping_to_manual
      ...
      it "can only transition to either #{previous_state}, #{next_state}, or #{alt_state}" do
        permitted_states = Helpers::EditWorkflowStates.permitted_states(subject)
