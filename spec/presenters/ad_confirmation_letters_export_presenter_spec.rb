...
RSpec.describe AdConfirmationLettersExportPresenter do
  let(:ad_confirmation_letters_export) { build(:ad_confirmation_letters_export) }
  subject(:presenter) { described_class.new(ad_confirmation_letters_export) }

  describe "#downloadable?" do
    before do
      ad_confirmation_letters_export.status = status
    end
    ...
    context "when the export succeded" do
      ...
      before do
        ad_confirmation_letters_export.number_of_letters = number_of_letters
      end
  ...
  describe "#print_label" do
    ...
    context "when the export succeded" do
      ...
      context "when the export has been printed" do
        context "when the email is in the format name_surname" do
          it "returns a label with the name of the person that printed it" do
            ad_confirmation_letters_export.printed_by = "katherine_johnson@nasa.org.uk"
            ad_confirmation_letters_export.printed_on = Date.today
        ...
        context "when the email is in the format name.surname" do
          it "returns a label with the name of the person that printed it" do
            ad_confirmation_letters_export.printed_by = "katherine.johnson@nasa.org.uk"
            ad_confirmation_letters_export.printed_on = Date.today
      ...
      context "when the export has not been printed" do
        ...
        context "when the number of letters is positive" do
          it "returns nil" do
            ad_confirmation_letters_export.number_of_letters = 4
            expect(presenter.print_label).to be_nil
          end
        end
      end
